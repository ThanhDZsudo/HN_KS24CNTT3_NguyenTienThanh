...
select room_type, count(*) as `tổng số phòng`
from rooms
group by room_type;
...
select g.guest_name, count(b.booking_id) as `số lần đặt phòng`
from guests g
...
select r.room_id, r.room_type, 
    sum((check_out - check_in) * price_per_day ) as `tổng doanh thu mõi phòng`
from bookings b 
join rooms r on b.room_id = r.room_id group by r.room_id;

-- ● Hiển thị tổng doanh thu của từng loại phòng
select r.room_type, 
       sum((b.check_out - b.check_in) * r.price_per_day) as `doanh thu từng loại phòng`
from bookings b
join rooms r on b.room_id = r.room_id group by r.room_type;

-- ● Tìm những khách đã đặt phòng từ 2 lần trở lên
select g.guest_id, g.guest_name, count(b.booking_id) as `sô lượt đặt`
from guests g
...
select r.room_type, count(b.booking_id) as `số lượt ddặt`
from rooms r join bookings b on r.room_id = b.room_id
group by r.room_type
order by `số lượt ddặt` desc
limit 1;
